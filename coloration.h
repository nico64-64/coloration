...
p_avance p_avances[] =
{
	{"-", "", '0', NULL, NULL, FALSE}, //Ce faux paramètre avancé doit toujours rester vide. Il sert uniquement à décaler la liste pour qu'elle commence à 1.
	{"CONFIRMER_QUITTER", "Demander une confirmation avant de quitter si le fichier n'est pas enregistré.", 'b', (int*) &confirmation_quitter, NULL, FALSE},
	{"CMD_COMPILER", "Commande envoyée pour \"compiler la liste\" (voir manuel d'aide).", 's', NULL, &cmd_compiler_liste[0], TRUE},
	{"SUPPORT_SOURIS", "Activer la prise en charge interactive de la souris (sur les terminaux compatibles).", 'b', (int*) &souris_activee, NULL, TRUE},
	#ifdef _ACCENTS_H
	{"FACCENTS", "Nom et emplacement du fichier de sauvegarde des accents.", 's', NULL, &nom_faccents[0], TRUE},
	#endif
	{"ERRLOG", "Logguer les erreurs dans un fichier txt.", 'b', (int*) &err_log, NULL, FALSE},
	{"FERREUR", "Nom et emplacement du fichier de log des erreurs (si activé).", 's', NULL, &nom_ferreur[0], FALSE},
	{"-", "Nom et emplacement du fichier de configuration de l'éditeur.", 's', NULL, &nom_fconfig[0], FALSE},
};
#ifdef _ACCENTS_H
const int nbre_p_avances = 7;
#else
const int nbre_p_avances = 6;
#endif
...
bool demande_quitter (); //demande à l'utilisateur s'il veut vraiment quitter et offre la possibilité d'enregistrer avant
...
int souris (MEVENT mev); //transforme un event de souris en l'event clavier correspondant
